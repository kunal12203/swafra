...
function config(args: string[]) {
  ...
  if (args.length === 0) {
    if (existsSync(configPath)) {
      try {
        ...
        if (cfg.llm_base_url) console.log(`     Base URL:   ${cfg.llm_base_url}`);
        console.log();
      } catch {
  ...
  for (let i = 0; i < args.length; i++) {
    if ((args[i] === "--provider" || args[i] === "-p") && args[i + 1]) { provider = args[++i]; }
    else if ((args[i] === "--key" || args[i] === "-k") && args[i + 1]) { apiKey = args[++i]; }
    else if ((args[i] === "--url" || args[i] === "-u") && args[i + 1]) { baseUrl = args[++i]; }
    else if ((args[i] === "--model" || args[i] === "-m") && args[i + 1]) { model = args[++i]; }
    else if (args[i] === "--clear") {
      if (existsSync(configPath)) {
        const cfg = JSON.parse(readFileSync(configPath, "utf-8"));
        delete cfg.llm_provider; delete cfg.llm_api_key;
        delete cfg.llm_base_url; delete cfg.llm_model;
  ...
  if (baseUrl) cfg.llm_base_url = baseUrl; else delete cfg.llm_base_url;
  if (model) cfg.llm_model = model; else delete cfg.llm_model;
